...
# GNU make way.
MAKEOVERRIDES = SHELL="$(SHELL)" QT_VERSION="$(QT_VERSION)" INSTALL_DIR="$(INSTALL_DIR)" INSTALL_FILE="$(INSTALL_FILE)" INSTALL_SYMLINK="$(INSTALL_SYMLINK)" INSTALL_PROGRAM="$(INSTALL_PROGRAM)" RM_FILE="$(RM_FILE)" RM_DIR="$(RM_DIR)" DESTDIR="$(DESTDIR)" PREFIX="$(PREFIX)" ETCDIR="$(ETCDIR)" BINDIR="$(BINDIR)" SHAREDIR="$(SHAREDIR)" MANDIR="$(MANDIR)" QMAKE_BINARY="$(QMAKE_BINARY)" LRELEASE_BINARY="$(LRELEASE_BINARY)" QMAKE_OPTS="$(QMAKE_OPTS)" LDFLAGS="$(LDFLAGS)" LIBS="$(LIBS)"
...
# OpenBSD way.
.MAKEFLAGS = SHELL="$(SHELL)" QT_VERSION="$(QT_VERSION)" INSTALL_DIR="$(INSTALL_DIR)" INSTALL_FILE="$(INSTALL_FILE)" INSTALL_SYMLINK="$(INSTALL_SYMLINK)" INSTALL_PROGRAM="$(INSTALL_PROGRAM)" RM_FILE="$(RM_FILE)" RM_DIR="$(RM_DIR)" DESTDIR="$(DESTDIR)" PREFIX="$(PREFIX)" ETCDIR="$(ETCDIR)" BINDIR="$(BINDIR)" SHAREDIR="$(SHAREDIR)" MANDIR="$(MANDIR)" QMAKE_BINARY="$(QMAKE_BINARY)" LRELEASE_BINARY="$(LRELEASE_BINARY)" QMAKE_OPTS="$(QMAKE_OPTS)" LDFLAGS="$(LDFLAGS)" LIBS="$(LIBS)"
